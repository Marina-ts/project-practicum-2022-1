...
export default class Basket {
    ...
    renderItem(item) {
        ...
        const itemPrice = isDiscount ?
            `<div class="basket-card__old-price" product-old-price>${item.oldPrice} ₽</div>
             <div class="basket-card__current-price" product-price>${item.price} ₽</div>` : `<div class="basket-card__current-price" product-price>${item.price} ₽</div>`
    ...
    renderInfo(data) {
        ...
        this.basketInfo.innerHTML = `
            <div class="basket__info-order">
                <span class="basket__info-order-heading">
                Информация о заказе
                </span>
    ...
    initChangeProduct() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', async (e) => {
                ...
                const count = +e.path[3].querySelector('[basket-count]').value
                const productPrice = +e.path[4].querySelector('[product-price]').innerText.replace('₽', '')
                const isDiscount = !!e.path[4].querySelector('[product-old-price]') && true;

                if(isDiscount) {
                    productOldPrice = +e.path[4].querySelector('[product-old-price]').innerText.replace('₽', '')
                }

                await this.deleteProduct(id, count, productPrice, productOldPrice)
            })
        })
    ...
    async deleteProduct(id, count, productPrice, productOldPrice) {
        ...
        if (result.ok) {
            ...
            const discountPrice = productOldPrice - productPrice
    ...
    updateBasketLength() {
        document.getElementById('basket-length').innerText = this.basketLength
    }
}